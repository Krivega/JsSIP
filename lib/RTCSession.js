...
const sequentPromises = require('sequent-promises').default;
...
function setDegradationPreference(sender, degradationPreference)
{
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    const mediaConstraints = Utils.cloneObject(options.mediaConstraints, {
      audio : options.audioMode !== 'recvonly',
      video : options.videoMode !== 'recvonly'
    });
    ...
    this._sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream, options.degradationPreference);
  }
  ...
  answer(options = {})
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          return this._addMediaStream(stream, 'getTracks', options.degradationPreference);
        }
      })
  ...
  terminate(options = {})
  {
    ...
    switch (this._status)
    {
      ...
      case C.STATUS_1XX_RECEIVED:
        ...
        {
          const error = new TypeError(`Invalid status_code: ${status_code}`);

          if (eventHandlers.failed)
          {
            eventHandlers.failed(error);
          }
  ...
  terminateAsync(options = {})
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      this.terminate({...options, eventHandlers});
    });  
  }  
  ...
  replaceMediaStream(stream, { deleteExisting = true, addMissing = true, forceRenegotiation = false } = {})
  {
  ...
  _addMediaStream(stream, action, degradationPreference)
  {
    return Promise.all(stream[action]().map((track) =>
    {
      const sender = this._connection.addTrack(track, stream);

      if (degradationPreference && track.kind==='video')
      {
        return setDegradationPreference(sender, degradationPreference);
      }
  ...
  _addOrReplacePresentationMediaStream(stream, degradationPreference)
  {
    ...
    {
      return this._addMediaStream(stream, 'getVideoTracks', degradationPreference);
    }
  }
  ...
  startPresentation(stream, isNeedReinvite=true, degradationPreference)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      const isNeedReplaceTracks = this._ua.configuration.sdp_semantics === 'unified-plan';
      ...
      Promise.resolve().then(() =>
      {
        ...
        {
          return this._addOrReplacePresentationMediaStream(stream, degradationPreference);
        }
            ...
            this._sendReinvite({
              eventHandlers : {
                ...
                failed    : () =>
                {
                  ...
                  {
                    this._removeMediaStream(stream);
                  }
   ...
   */
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream, degradationPreference)
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          return this._addMediaStream(stream, 'getTracks', degradationPreference);
        }
